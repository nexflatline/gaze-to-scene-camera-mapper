...
def get_frame_gaze_data(frame_idx, eye_frame, dlc_data, dlc_bodyparts,
    ...
    Args:
        ...
        comp_enabled: Whether stabilization is enabled (bool)
        stab_template, stab_ref_point, stab_curr_point, comp_axes: Stabilization params
        comp_axes: Dict with 'x'/'y' bools
    ...
    Returns:
        Tuple of (data dict, new_stab_curr_point). new_stab_curr_point should be
        passed back in as stab_curr_point on the next frame.
    """
    data = {
    ...
    new_stab_curr = stab_curr_point

    if eye_frame is not None and comp_enabled and stab_template is not None:
        stab_stats, new_pt = get_image_compensation_func(
    ...
    for side in ["left", "right"]:
        ...
        if eye_frame is not None and comp_enabled and stab_template is not None:
            pupil_stab, _, _ = apply_compensation_func(
    ...
    if best_eye in eyes_data and eye_frame is not None:
