...
def predict_gaze(pupil_x, pupil_y, side, frame_bgr, gaze_regressors, reference_centers, 
    ...
    Args:
        ...
        stab_template, stab_ref_point, stab_curr_point, comp_axes: Compensation params
        comp_axes: Dict with 'x'/'y' bools

    Returns:
        Tuple of ((gaze_x, gaze_y), (dx, dy, da)) or (None, stats) if prediction fails.
        Gaze coordinates are floats (round only when drawing).
    """
    models = gaze_regressors.get(side)
    if models is None:
        return None, (0, 0, 0)

    result = apply_compensation_func(
        pupil_x, pupil_y, frame_bgr, comp_enabled, stab_template,
        stab_ref_point, stab_curr_point, comp_axes
    )
    # Support both (coords, stats) and (coords, stats, new_curr_point)
    (adj_px, adj_py), stats = result[0], result[1]
