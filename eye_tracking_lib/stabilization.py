...
def get_image_compensation(current_bgr, stab_template, stab_ref_point, stab_curr_point, comp_axes):
    ...
    Args:
        ...
        comp_axes: Dict with 'x'/'y' bools indicating which axes to compensate
    ...
    if not comp_axes.get("x", True):
        dx = 0.0
    if not comp_axes.get("y", True):
        dy = 0.0
...
def apply_compensation_to_pupil(pupil_x, pupil_y, frame_bgr, comp_enabled, stab_template,
    ...
    Args:
        ...
        comp_enabled: Whether compensation is enabled (bool)
    ...
    Returns:
        Tuple of ((final_x, final_y), (dx, dy, da), new_curr_point)
    """
    if not comp_enabled or stab_template is None:
        return (pupil_x, pupil_y), (0, 0, 0), stab_curr_point

    stats, new_curr_point = get_image_compensation(
        frame_bgr, stab_template, stab_ref_point, stab_curr_point, comp_axes
    )
    dx, dy, da = stats
    ...
    return (final_x, final_y), stats, new_curr_point
