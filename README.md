...
Open an **Anaconda Prompt** (Windows) or a terminal (macOS/Linux) and create a new environment with Python 3.9. Use **conda-forge** (not the Anaconda `defaults` channel):

```bash
conda create -n gaze-mapper -c conda-forge python=3.9
...
Imagine the pupil as a clock face. Label 8 points around its rim:
...
| Column                              | Description                                                   |
| ----------------------------------- | ------------------------------------------------------------- |
| `frame_idx`                         | Zero-based video frame index                                  |
| `best_eye`                          | Which eye was used (`L` or `R`); empty if neither was valid   |
...
**Gaze seems scaled oddly after calibration**  
→ Gaze features assume a pupil travel radius of about 100 pixels in the eye camera. Calibration still fits scene coordinates, but very zoomed-in or zoomed-out eyes may need more calibration points.
...
→ Or double-click the row and assign the face/scene videos manually.
...
If you use this tool in a publication, please cite this repository and DeepLabCut.
...
Nature Neuroscience, 21(9), 1281–1289. https://doi.org/10.1038/s41593-018-0209-y
```
...
Copyright (c) 2026 Rafael Bretas, Center for Information and Neural Networks (CiNet), National Institute of Information and Communications Technology (NICT)
