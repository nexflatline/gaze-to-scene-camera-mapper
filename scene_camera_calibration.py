...
class DLCSceneCalibrator3D:
    ...
    def setup_bindings(self):
        self.root.bind('<Right>', lambda e: self.seek(30))
        self.root.bind('<Left>', lambda e: self.seek(-30))
    ...
    def toggle_landmark_mode(self):
        ...
        if self.defining_landmark:
            ...
            self.status_label.config(text="Click a skull-fixed feature (e.g. bridge of the nose) on the Eye Video.")
        else:
    ...
    def _comp_axes_bools(self):
        return {"x": self.comp_axes["x"].get(), "y": self.comp_axes["y"].get()}
    ...
    def get_image_compensation(self, current_bgr):
        ...
        stats, new_curr_point = get_image_compensation(
            ...
            self.stab_curr_point, self._comp_axes_bools()
        )
        if new_curr_point:
    ...
    def apply_compensation_to_pupil(self, pupil_x, pupil_y, frame_bgr):
        ...
        coords, stats, new_curr = apply_compensation_to_pupil(
            pupil_x, pupil_y, frame_bgr, self.comp_enabled.get(), self.stab_template,
            self.stab_ref_point, self.stab_curr_point, self._comp_axes_bools()
        )
        if new_curr:
    ...
    def predict_gaze(self, pupil_x, pupil_y, side, frame_bgr):
        ...
        return predict_gaze_module(
            ...
            self.eye_models, apply_compensation_to_pupil, self.comp_enabled.get(), self.stab_template,
            self.stab_ref_point, self.stab_curr_point, self._comp_axes_bools()
        )
    ...
    def refresh_displays(self):
        ...
        if self.raw_scene_frame is not None:
            ...
            for side, (pupil, _) in eyes.items():
                ...
                if pred:
                    predictions[side] = self._pixel(pred)
    ...
    def compute_calibration(self):
        ...
        if not any(v is not None for v in self.gaze_regressors.values()):
            messagebox.showerror(
                "Error",
                "Calibration failed: not enough valid points for either eye after filtering.",
            )
            return
        ...
        try:
            for i in range(self.total_frames):
                ...
                for side, (pupil, _) in eyes_data.items():
                    ...
                    if res:
                        predictions[side] = self._pixel(res)
    ...
    def get_frame_gaze_data(self, frame_idx, eye_frame):
        ...
        data, new_curr = get_frame_gaze_data_module(
            frame_idx, eye_frame, self.dlc_data, self.dlc_bodyparts,
            self.comp_enabled.get(), self.stab_template, self.stab_ref_point, self.stab_curr_point,
            self._comp_axes_bools(),
