...
class BatchProcessorGUI:
    ...
    def create_ui(self):
        ...
        ttk.Label(btn_bar, text=" * Auto-matches videos from the CSV name. Double-click a row to assign videos manually.", foreground="gray").pack(side=tk.LEFT, padx=10)
    ...
    def on_tree_double_click(self, event):
        ...
        if index < 0 or index >= len(self.file_registry):
            return
        entry = self.file_registry[index]

        video_types = [("Video", "*.mp4 *.avi *.mov *.mkv"), ("All files", "*.*")]
        face = filedialog.askopenfilename(title="Select face/eye video", filetypes=video_types)
        if face:
            entry["face"] = face
        scene = filedialog.askopenfilename(title="Select scene video", filetypes=video_types)
        if scene:
            entry["scene"] = scene

        if entry["face"] and entry["scene"]:
            entry["status"] = "Ready"
        else:
            entry["status"] = "MISSING VIDEO"

        self.tree.item(item_id, values=(
            os.path.basename(entry["csv"]),
            os.path.basename(entry["face"]) if entry["face"] else "NOT FOUND",
            os.path.basename(entry["scene"]) if entry["scene"] else "NOT FOUND",
            entry["status"],
        ))
    ...
    def process_batch(self):
        ...
        # Snapshot UI values on the main thread; worker uses plain bools only.
        config = {
            'export_csv': self.opt_export_csv.get(),
            'export_video': self.opt_export_video.get(),
            'enable_stab': self.opt_enable_stab.get(),
            'comp_axes': {'x': self.opt_comp_x.get(), 'y': self.opt_comp_y.get()},
    ...
    def process_single_file(self, entry, config):
        ...
        for i in range(total_frames):
            ...
            frame_data, new_curr = get_frame_gaze_data_module(
            ...
            if config['export_video'] and video_writer:
                ...
                if pd.notna(frame_data['best_eye_gaze_x']):
                    gaze_pt = (
                        int(round(frame_data['best_eye_gaze_x'])),
                        int(round(frame_data['best_eye_gaze_y'])),
                    )
